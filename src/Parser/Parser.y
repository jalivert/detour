...
Consts      ::  { [(String, Type'Scheme)] }
            :   TypeAnn                       { let { (ns, ts) = $1 } in map (,ts) ns }
            |   TypeAnn ',' Consts            { let { (ns, ts) = $1 } in map (,ts) ns ++ $3 }
...
Binders     ::  { [(String, Type'Scheme)] }
            ...
            |   TypeAnn Binders             {% do
                                                ...
                                                ; let (ns, t) = $1
                                                ; let cs = map (,t) ns
                                                ; alexSetUserState s{ bound = (ns ++ b) : bs }
                                                ; return (cs ++ $2) } }


TypeAnn   ::  { ([String], Type'Scheme) }
          :   '(' IDENT ':' Type ')'      { ([$2], Forall'T [] $4) }
          -- |   IDENT ':' Type              { ([$1], Forall'T [] $3) }
          |   '(' Idents ':' Type ')'     { ($2, Forall'T [] $4) }
          |   IDENT                       {% do
                                              { n <- fresh'name
...
Term        ::  { Term }
            ...
            |   '_' IDENT 'ᶜ' TermArgsM     { App (S.C ('_' : $2)) (fromMaybe [] $4) }
            ...
            |   '_' IDENT TermArgsM           {%  case $3 of
                                                { Just terms -> return $! App (S.C ('_' : $2)) terms
                                                ; Nothing -> do
                                                  { let name = '_' : $2
                                                  ; s <- alexGetUserState
                                                  ; let cons = A.constants s
                                                  ...
                                                  ; let alis = aliases s
                                                  ; let bs = bound s
                                                  ; let is'constant = name `elem` cons
                                                  ; let is'upper = isUpper (head name)
                                                  ; let is'bound = List.any (name `elem`) bs
                                                  ; let is'rigid = List.any (name `elem`) rs
                                                  ; if is'bound
                                                    then return (Bound'Var name)
                                                    else  if is'rigid
                                                          then return (Var (Rigid (R name)))
                                                          else  if is'upper || is'constant
                                                                then return (Const name)
                                                                else  case List.lookup name alis of
                                                                      { Just tm -> do { return tm }
                                                                      ; Nothing -> return $! Free'Var name } } } }
...
Proof       ::  { [Judgment] }
            ...
            |   '_' PropVar ':=' Formula Proof
                                            { (JD.Alias ('_' : $2) $4) : $5 }
            |   Name ':' 'prove' Formula Proof
                                            { (JD.Prove $1 $4) : $5 }
            |   'prove' Formula Proof       { (JD.Prove Nothing $2) : $3 }
            |   {- empty -}                 { [] }
                                            -- ; $$.outScope = empty }
...
parse'proof :: AlexUserState -> String -> Either String (AlexState, [Judgment])
parse'proof st source = run'alex parseProof st'
  ...
  where 
        ...
        run'alex :: Alex a -> AlexState -> Either String (AlexState, a)
        run'alex (Alex f) st = f st

--  runAlex source parseProof


parse'command :: String -> Either String Rule
parse'command source = runAlex source parseCommand
