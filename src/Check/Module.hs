...
import Check.Check ( Check, because, Q(..), inform'about )
import Check.Environment ( Environment(assert'scope, theorems, current'theorem) )
...
check'module Module { M.name, M.constants, M.aliases, M.axioms, M.syntax, M.judgments, M.theorems = thrms } = do
  ...
  where clean'state :: Monad m => Check m ()
        ...
        try'theorem th = do
          let name = T.name th
          clean'state
          -- inform'about ("I am checking theorem `" ++ name ++ "'.")
          res <- tryError  (local (\ e -> e{ current'theorem = Just name })
                                  (check'theorem th))
          case res of
